...
//            $params[':operatorid'] = intval($_W['cashieruser']['operator']['id']);
//        }else{
...
//            pdo_query("DELETE FROM " . tablename('ewei_shop_cashier_pay_log') . " WHERE id IN (".implode(',',array_keys($goods)).")");
        ...
        $operator = pdo_fetchall("SELECT id,title FROM ".tablename('ewei_shop_cashier_operator')." WHERE uniacid=:uniacid AND cashierid=:cashierid ORDER BY id DESC",array(
            ':uniacid'=>$_W['uniacid'],
            ':cashierid'=>$_W['cashierid']
        ),'id');
        ...
        $total_money = (float)pdo_fetchcolumn("select sum(money+deduction) from " . tablename('ewei_shop_cashier_pay_log') . " where uniacid=:uniacid {$condition}", $params);
    ...
    function change() {
        ...
        /*WHERE id=:id AND uniacid=:uniacid AND cashierid=:cashierid';*/
        /*$sql = ' SELECT g.* FROM ' . tablename('ewei_shop_cashier_pay_log'). " l"
            . " left join " . tablename('ewei_shop_cashier_pay_log_goods') . " r on l.id = r.logid "
            . " left join " . tablename('ewei_shop_cashier_goods') . " g on g.id = r.goodsid "
            . " WHERE l.id=:id AND l.uniacid=:uniacid AND l.cashierid=:cashierid ";

        $list = pdo_fetchall($sql, $canshu);*/

        $sql = ' SELECT g.*,og.total,o.ispackage,og.price as orderprice,og.goodssn,og.productsn,og.realprice,o.goodsprice,o.price,o.olddispatchprice,og.optionname,l.status as state FROM ' . tablename('ewei_shop_cashier_pay_log'). " l"
            . " left join " . tablename('ewei_shop_order') . " o on o.id = l.orderid "
            . " left join " . tablename('ewei_shop_order_goods') . " og on og.orderid = o.id "
            . " left join " . tablename('ewei_shop_goods') . " g on g.id = og.goodsid "
            . " WHERE l.id=:id AND l.uniacid=:uniacid AND l.cashierid=:cashierid  AND l.orderid !=''";
        $list = pdo_fetchall($sql, $canshu);


        $sql2 = ' SELECT g.image as thumb,g.title,r.total,g.price as marketprice,g.goodssn,r.price as orderprice,r.price as realprice,l.status as state FROM ' . tablename('ewei_shop_cashier_goods'). " g"
            . " left join " . tablename('ewei_shop_cashier_pay_log_goods') . " r on r.goodsid = g.id "
            . " left join " . tablename('ewei_shop_cashier_pay_log') . " l on l.id = r.logid "
            . " WHERE l.id=:id AND l.uniacid=:uniacid AND l.cashierid=:cashierid ";
