...
//            }
        ...
        if ($_GPC['export'] == '1') {
            ...
            m('excel')->export($list, array(
                "title" => "分销商统计数据-" . date('Y-m-d-H-i', time()),
                "columns" => $columns
            ));
        }
    ...
    function user() {
        ...
        if ($hasagent) {
            ...
            $list = pdo_fetchall("select dm.*,p.nickname as parentname,p.avatar as parentavatar  from " . tablename('ewei_shop_member') . " dm "
                    . " left join " . tablename('ewei_shop_member') . " p on p.id = dm.agentid "
                    . " left join " . tablename('mc_mapping_fans') . "f on f.openid=dm.openid  and f.uniacid={$_W['uniacid']}"
                    . " where dm.uniacid = " . $_W['uniacid'] . "  {$condition}   ORDER BY dm.agenttime desc limit " . ($pindex - 1) * $psize . ',' . $psize, $params);
