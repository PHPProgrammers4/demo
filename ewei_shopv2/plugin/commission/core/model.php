						...
						if ($selfbuy == 1) {
							foreach ($res as $key => $value) {
								if ($key == 0) {
									if ($tag == 'commission_order_pay') {
										$message = str_replace('[佣金金额]', $data['commission2'], $message);
										$message = str_replace('[下线层级]', '您的二级下级', $message);
									}
								}

								if ($key == 1) {
									if ($tag == 'commission_order_pay') {
										$message = str_replace('[佣金金额]', $data['commission3'], $message);
										$message = str_replace('[下线层级]', '您的三级下级', $message);
									}
								}
							}
						}
							...
							if ($selfbuy == 0) {
								foreach ($res as $key => $value) {
									if ($key == 0) {
										if ($tag == 'commission_order_pay') {
											$message = str_replace('[佣金金额]', $data['commission1'], $message);
											$message = str_replace('[下线层级]', '您的一级下级', $message);
										}
									}
...
感谢您的支持', 'color' => '#000000')
		...
		public function saveRelation($id, $pid, $level = 1, $later = array())
		{
			...
			$childs = pdo_fetchall('select * from ' . tablename('ewei_shop_commission_relation') . ' where pid = :pid', array(':pid' => $id));
