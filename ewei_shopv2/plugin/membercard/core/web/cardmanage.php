...
class Cardmanage_EweiShopV2Page extends PluginWebPage {
    function main() {
        ...
        if($list){
            foreach ($list as $key => $val){
                ...
                $equity = '';
                if($val['shipping']) $equity .= '免费包邮·';
                if($val['member_discount']) $equity .= $val['discount_rate'].'折优惠·';
                if($val['is_card_coupon']) $equity .= '开卡送券·';
                if($val['is_month_coupon']) $equity .= '每月领券·';
                $equity = trim($equity);
                if(!empty($equity)){
                    $equity = rtrim($equity,'·');
                }
    ...
    protected function post() {
        ...
        if ($_W['ispost']) {
            ...
            $data = array(
                'uniacid' => $_W['uniacid'],
                'name' => trim($_GPC['name']),
                'sort_order' => intval($_GPC['sort_order']),
                'card_style' => trim($_GPC['card_style']),
                'shipping' => intval($_GPC['shipping']),
                'member_discount' => intval($_GPC['member_discount']),
                'discount_rate' => floatval($_GPC['discount_rate']),
                'cardmodel' => $_GPC['cardmodel'],
                'discount' => intval($_GPC['discount']),
                'is_card_points' => intval($_GPC['is_card_points']),
                'card_points' => intval($_GPC['card_points']),
                'is_card_coupon' => intval($_GPC['is_card_coupon']),
                'is_month_points' => intval($_GPC['is_month_points']),
                'month_points' => intval($_GPC['month_points']),
                'is_month_coupon' => intval($_GPC['is_month_coupon']),
                'validate' => intval($_GPC['validate']),
            ...
            if($data['discount_rate'] && ($data['discount_rate']<=0 || $data['discount_rate']>=10)){
            ...
            if($data['is_card_coupon']){
                if(empty($_GPC['couponids'])) show_json( 0 ,'请选择开卡赠送的优惠券');
            ...
            if($data['is_month_coupon']){
                if(empty($_GPC['couponid'])) show_json( 0 ,'请选择每月赠送的优惠券');
            ...
            if($_GPC['validity'] == 1){
                if(empty($data['validate'])) show_json( 0 ,'请填写会员卡有效期');
                if($data['validate']<=0) show_json( 0 ,'请填写正确的有效期');
            }else{
                $data['validate'] = -1;
            }
            if($data['price']<0){
    ...
    function delete() {
        ...
        if (empty($item)) {
            ...
            //message('抱歉,会员卡不存在或者已经被删除', webUrl('membercard.cardmanage', array('op' => 'display')), 'error');
            show_json( 0 ,'抱歉,会员卡不存在或者已经被删除');
        }
        
        pdo_update('ewei_shop_member_card', array('isdelete' =>1,'del_time'=>time()), array('id' => $id, "uniacid" => $_W['uniacid']));//删除会员卡
        pdo_update('ewei_shop_member_card_history', array('isdelete' =>1,'del_time'=>time()), array('member_card_id' => $id, "uniacid" => $_W['uniacid']));//删除会员卡
    ...
    //查询优惠券
    function querycoupon($couponid = array()){
        global $_W,$_GPC;
        $cpinfo = array();
