...
//        $credit = m('member')->getCredit($openid, 'credit1');
        ...
        if (!empty($total)) {
            ...
            foreach ($list as &$row) {
                ...
                if( $row['type'] == 0 ){
                    if( $row['goodstype'] == 0 ){
                        if( $row['isverify'] == 1 ){
                            if( $row['status'] == 2 ){ $row['status_name'] = '待兑换'; }
                            if( $row['isreply'] == 1 ){
                                if( $row['status'] ==3 && $row['iscomment'] == 0){ $row['status_name'] = '待评价'; }
                                if( $row['status'] ==3 && $row['iscomment'] == 1){ $row['status_name'] = '追加评价'; }
                                if( $row['status'] ==3 && $row['iscomment'] == 2){ $row['status_name'] = '已完成'; }
                            }else{
                                if( $row['status'] ==3 ){ $row['status_name'] = '已完成'; }
                            }
                        }else{
                            if( $row['status'] ==2 && $row['addressid'] == 0){ $row['status_name'] = '已兑换'; }
                            if( $row['status'] ==2 && $row['addressid'] > 0 && $row['time_send'] == 0 ){ $row['status_name'] = '待发货'; }
                            if( $row['status'] ==3 && $row['addressid'] > 0 && $row['time_send'] == 0){ $row['status_name'] = '待发货'; }
                            if( $row['status'] ==3 && $row['time_send'] > 0 && $row['time_finish'] == 0 ){ $row['status_name'] = '待收货'; }
                            if( $row['isreply'] == 1){
                                if( $row['status'] ==3 && $row['time_finish'] > 0 && $row['iscomment'] == 0 ) { $row['status_name'] = '待评价'; }
                                if( $row['status'] ==3 && $row['time_finish'] > 0 && $row['iscomment'] == 1 ){ $row['status_name'] = '追加评价'; }
                                if( $row['status'] ==3 && $row['time_finish'] > 0 && $row['iscomment'] == 2 ){ $row['status_name'] = '已完成'; }
                            }else{
                                if( $row['status'] ==3){ $row['status_name'] = '已完成'; }
                            }
                        }
                            ...
                            if( $row['isreply'] == 1 ) {
                                ...
                                if ($row['status'] == 3){ $row['status_name'] = '已发放'; }
                            }
                        }else if( $row['goodstype']==2) {
                            if ( $row['isreply'] == 1 ) {
                                if($row['status'] == 3 && $row['iscomment'] == 0 ){ $row['status_name'] = '待评价'; }
                                if($row['status'] == 3 && $row['iscomment'] == 1 ){ $row['status_name'] = '追加评价'; }
                                if($row['status'] == 3 && $row['iscomment'] == 2 ){ $row['status_name'] = '已发放'; }
                            }else {
                                if ( $row['status'] == 3 ) { $row['status_name'] = '已发放'; }
                            }
                        }else if ($row['goodstype']==3) {
                            if ($row['status'] == 2) {  $row['status_name'] = '待领取';}
                            if ($row['isreply'] == 1 ){
                                if ($row['status'] == 3 && $row['iscomment'] == 0 ){ $row['status_name'] = '待评价';}
                                if ($row['status'] == 3 && $row['iscomment'] == 1 ){ $row['status_name'] = '追加评价';}
                                if ($row['status'] == 3 && $row['iscomment'] == 2 ){ $row['status_name'] = '已发放';}
                    ...
                    if( $row['goodstype'] == 0 ){
                        if( $row['isverify'] == 1 ){
                            ...
                            if( $row['status'] ==2 && $row['addressid'] == 0){ $row['status_name'] = '已中奖'; }
                            if( $row['status'] ==2 && $row['addressid'] > 0){ $row['status_name'] = '待收货'; }
                            if( $row['status'] ==2 && $row['addressid'] > 0 && $row['time_send'] == 0 ){ $row['status_name'] = '待发货'; }
                            if( $row['status'] ==3 && $row['addressid'] > 0 && $row['time_send'] == 0){ $row['status_name'] = '待发货'; }
                            if( $row['status'] ==3 && $row['time_send'] > 0 && $row['time_finish'] == 0 ){ $row['status_name'] = '待收货'; }
                            if( $row['isreply'] == 1){
                    ...
                    if ($row['status'] == 1 ){ $row['status_name'] = '暂未中奖';}
                }
    ...
    function paydispatch(){
        ...
        if($paytype == 'wechat'){
            ...
            $payinfo = array(
                'openid' => $_W['openid_wa'],
                'title' => $set['shop']['name'] . ( empty($goods['type']) ? "积分兑换" : '积分抽奖') . ' 支付运费单号:' . $dispatchno,
                'tid' => $tid,
                'fee' => $dispatchprice
            );
    ...
    function express()
    {
        ...
        $order = pdo_fetch("select expresscom,expresssn,addressid,status,express from " . tablename('ewei_shop_creditshop_log') . ' where id=:id and uniacid=:uniacid and openid=:openid limit 1'
        ...
        //商品信息
        $goods = pdo_fetchall("select og.goodsid,g.thumb  from " . tablename('ewei_shop_creditshop_log') . " og "
            . " left join " . tablename('ewei_shop_creditshop_goods') . " g on g.id=og.goodsid "
            . " where og.id=:orderid and og.uniacid=:uniacid ", array(':uniacid' => $uniacid, ':orderid' => $orderid));
