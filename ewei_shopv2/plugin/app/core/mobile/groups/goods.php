...
class Goods_EweiShopV2Page extends AppMobilePage
{
    public function main()
    {
        ...
        if (empty($id) || empty($goods)) {
            return app_error( '您查找的商品不存在或已删除' );set_medias();
        }
        if (!empty($goods['thumb_url'])) {
            $goods['thumb_url'] = array_merge(iunserializer($goods['thumb_url']));
        }
        $goods['fightnum'] = pdo_fetchcolumn("select count(1) from ".tablename('ewei_shop_groups_order')." where goodid = :goodid and uniacid = :uniacid and deleted = 0 and is_team = 1 and status > 0 ",
            array(":goodid"=>$goods['id'],':uniacid'=>$uniacid));
    ...
    public function openGroups()
    {
        ...
        $goods['fightnum'] = pdo_fetchcolumn("select count(1) from ".tablename('ewei_shop_groups_order')." where goodid = :goodid and uniacid = :uniacid and deleted = 0 and is_team = 1 and status > 0 ",
            array(":goodid"=>$goods['id'],':uniacid'=>$_W['uniacid']));
        $goods['fightnum'] = $goods['teamnum'] + $goods['fightnum'];
        $goods = set_medias($goods, 'thumb');
        ...
        foreach($teams as $key => $value){
            $value['fightnum'] = pdo_fetchcolumn("select count(1) from ".tablename('ewei_shop_groups_order')." where goodid = :goodid and uniacid = :uniacid and deleted = 0 and is_team = 1 and status > 0 ",
                array(":goodid"=>$value['id'],':uniacid'=>$_W['uniacid']));
...
//        var_Dump( $ladder );die;
    ...
    public function fight_groups()
    {
        ...
        if( !empty($goods) ){
            $goods['fightnum'] = pdo_fetchcolumn("select count(1) from ".tablename('ewei_shop_groups_order')." where goodid = :goodid and uniacid = :uniacid and deleted = 0 and is_team = 1 and status > 0 and ladder_id = :ladder_id ",
                array(":goodid"=>$goods['id'],':uniacid'=>$uniacid,':ladder_id' => $ladder_id));
            $goods['fightnum'] = $goods['teamnum'] + $goods['fightnum'];
        }
        ...
        $teams = pdo_fetchall("select o.paytime,o.id,o.goodid,o.teamid,m.nickname,m.realname,m.mobile,m.avatar,g.endtime,g.groupnum,g.thumb_url from ".tablename('ewei_shop_groups_order')." as o
				...
				where o.goodid = :goodid and o.uniacid = :uniacid and o.openid != :openid and o.deleted = 0 and o.heads = 1 and o.paytime > 0 and o.success = 0 and o.ladder_id = :ladder_id limit 10 ",
            array(":goodid"=>$goods['id'],':uniacid'=>$uniacid,':openid'=>$openid,'ladder_id' => $ladder_id));

        foreach($teams as $key => $value){
            $num = pdo_fetchcolumn("select count(1) from ".tablename('ewei_shop_groups_order')." where uniacid = :uniacid and deleted = 0 and teamid = :teamid and status > 0 ",
                array(":teamid"=>$value['teamid'],':uniacid'=>$uniacid));
            if( !empty( $ladder_id ) ){
...
//dump( array( 'data' => $goods , 'other' => $teams ));die;
 ...
 * */
    function goodsCheck(){
        ...
        $ordernum = pdo_fetchcolumn('select count(1) from ' . tablename('ewei_shop_groups_order') . " as o
            ...
            array(':openid'=>$open_id,':status'=>0,':goodid'=>$id,':uniacid'=>$uniacid));
        ...
        $order = pdo_fetch("select * from " . tablename('ewei_shop_groups_order') . '
                where goodid = :goodid and status >= 0  and openid = :openid and uniacid = :uniacid and success = 0 and deleted = 0 order by createtime desc',
            array(':goodid' => $id, ':openid' => $open_id, ':uniacid' => $uniacid));
//        dump( $order && $order['status'] );die( '123' );
