...
class Pay_EweiShopV2Page extends AppMobilePage{
    ...
    function main(){
        ...
        if (isset($set['pay']) && $set['pay']['credit'] == 1 ) {/*&& $member['credit2'] >= $log['fee']*/
            if ($order['deductcredit2'] <= 0) { //未有余额抵扣
                $credit = array(
                    'success' => true,
        ...
        if (!empty($set['pay']['wxapp']) && ($order['price']+$order['freight'])>0 && $this->iswxapp){
            $payinfo = array(
                'openid' => $_W['openid_wa'],
    ...
    function complete() {
        ...
        if ($type == 'credit') {
            ...
            //余额
            $result = m('member')->setCredit($openid, 'credit2', -$fee, array($member['uid'], $_W['shopset']['shop']['name'].'消费' . $fee));
