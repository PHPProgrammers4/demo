...
//                }
                ...
                where uniacid = :uniacid and id = :giftid and status = 1 and starttime <= " . time() . " and endtime >= " . time() . " ",array(':uniacid'=>$uniacid,':giftid'=>$giftid));
                    if (!empty($gift['giftgoodsid'])) {
                        $giftGoodsid = explode(',', $gift['giftgoodsid']);
                        if ($giftGoodsid) {
                            foreach ($giftGoodsid as $key => $value) {
            ...
            $card_info = array();               //订单会员卡信息
            //计算会员卡------------began
            $plugin_membercard = p('membercard');
            if(!$plugin_membercard){
                $canusecard = false;
            }
            $availablecard_count = 0;           //可用的会员卡数量
            $default_cardid = 0;                //默认使用的会员卡的id
            $carddiscountprice = 0;             //会员卡折掉的金额
            //$pure_totalprice = $realprice - $dispatch_price;    //去掉运费之后的价格
            $pure_totalprice = $realprice;      //去掉运费之后的价格
            $card_free_dispatch = false;        //是否会员卡免邮
            //如果有默认选中的会员卡
            if($canusecard){
            ...
            //$realprice = $pure_totalprice + $dispatch_price;
            //如果本来来已经打了会员折扣,但是会员卡是禁用折上折的话,需付价格要把折掉的价格加回去
            if($card_info['old_discountprice'] >0 && $discountprice == 0){
                $realprice += $card_info['old_discountprice'];
            }
            //如果本来来已经有了促销折扣,但是会员卡是禁用折上折的话,需付价格要把折掉的价格加回去
            ...
            if ($g['seckillinfo'] && $g['seckillinfo']['status'] == 0) {
                ...
                if ($giftid) {
                    ...
                    /*$has_hift = false;
                    if($giftdata['activity'] == 1 ){
                        if($giftdata['orderprice'] >= $realprice) $has_hift = true;
                    }else{
                        $has_hift = true;
                    }*/
        ...
        foreach($goods_list as $k => $v){
            ...
            foreach($v['goods'] as $g){
                $allgoods[$k]['goods'][] = array(
                    ...
                    'minbuy'=>$g['minbuy'],
                    'promotionprice' => ($g['marketprice'] > $g['unitprice']? (float)$g['marketprice']: (float)$g['unitprice']) - $g['isdiscountprice'],
                );
            }
        }
...
//            'storeInfo'=>!empty($store_list)? $store_list[0]:false,
    ...
    function getcouponprice()
    {
        ...
        $real_price = $_GPC['real_price'];      //订单需付实际价格
                ...
                foreach ($goodsarr as &$g) {
                    ...
                    if($_GPC['bargain_id'] > 0){
                        ...
                        $sql = 'SELECT id as goodsid,title,type, weight,total,issendfree,isnodiscount, thumb,marketprice,cash,isverify,isforceverifystore,goodssn,productsn,sales,istime,'
                            ...
                            . ' timestart,timeend,usermaxbuy,maxbuy,unit,buylevels,buygroups,deleted,status,deduct,ispresell,presellprice,preselltimeend,manydeduct,`virtual`,'
...
//                            $data['marketprice'] = $option['marketprice'];
                ...
                //$coupondecprice = floatval($_GPC['coupondecprice']);
                $plugin_membercard = p('membercard');
                $card_info = array();
                $carddiscountprice = 0;             //会员卡折掉的金额
                $carddiscount_rate = 0;             //会员卡折
                ...
                if($plugin_membercard && $cardid){
                    ...
                    if($card_result){
                        ...
                        $card_info['old_discountprice'] = $discountprice;
                        $discountprice = $card_result['discountprice'];
                        //$card_info['realprice'] = $pure_totalprice;
                        $card_info['old_isdiscountprice'] = $isdiscountprice;
                        $isdiscountprice = $card_result['isdiscountprice'];
                ...
                if($card_info['dispatch_price'] >0 && $dispatch_price == 0){
                    //$realprice -= $card_info['dispatch_price'];
                    $card_free_dispatch = true;
                }
                if($card_info['old_discountprice'] >0 && $discountprice == 0){
                    $realprice += $card_info['old_discountprice'];
        ...
        if($couponid){
            ...
            //此处目的不明确，计算抵扣时有错误，先注释 2018-12-22 suniw
...
//        }
        ...
        $goods = $goods_tmp;
        $goods[0]['bargain_id'] = $_GPC['bargain_id'];//砍价订单的价格传递
        ...
        foreach ($goods as $g) {
            ...
            if (!empty($optionid)) {
                ...
                if (empty($option)) {
                    //如果该规格被删除,就不能下单
                    app_error(1, $data['title'] . "规格或已被删除 ,请重新选择规格");
                }
                if ($data['seckillinfo'] && $data['seckillinfo']['status'] == 0) {
                } else {
                    if ($option['stock'] != -1) {
        ...
        if($plugin_membercard && $cardid){
            ...
            if($card_result){
                $card_info['dispatch_price'] = $dispatch_price;
                $dispatch_price = $card_result['dispatch_price'];
                //$card_info['before_price'] = $pure_totalprice;
                $carddiscountprice = $card_result['carddiscountprice'];
                $card_info['old_discountprice'] = $discountprice;
                $discountprice = $card_result['discountprice'];
                $card_info['old_isdiscountprice'] = $isdiscountprice;
        ...
        //app_json(array('saleset' => $deductenough));
        ...
        if($data['type'] == 3){
            ...
            if(is_array($virtual_res) && $virtual_res['message']){
                app_error(1, $virtual_res['message']);
            }
        }
    ...
    function getcardprice()
    {
        ...
        $goodsprice = $_GPC['goodsprice'];
        $discountprice = $_GPC['discountprice'];        //会员折扣
    ...
    /*//计算会员卡价格
    function caculatecard($cardid,$dispatch_price,$totalprice,$discountprice,$isSubmit =0){
        global $_W;
        ...
        if($card['member_discount'] && $discount_rate>0){
            $carddiscountprice = round($totalprice * (10-$discount_rate) * 0.1,2);
            if($discountprice>0 && empty($card['discount'])){
                $carddiscountprice = 0;
                $discount_rate = 0;
            }
    ...
    //计算会员卡价格
    function caculatecard($cardid,$dispatch_price,$totalprice,$discountprice=0,$isdiscountprice =0){
        global $_W;
