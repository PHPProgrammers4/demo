...
class Member_EweiShopV2Page extends AppMobileAuthPage {
    ...
    public function get_list() {
        ...
        return app_json(array(
            ...
            'page'=>$pindex,
            'perm'=>array(
                'member_view'=>cv('member.list.view'),
                'member_edit'=>cv('member.list.edit'),
            )
        ));
    }
    ...
    public function get_detail() {
        ...
        $result = array(
            'member'=>array(
                ...
                'followedtext'=>$member['followedtext'],

                'credit2'=>$member['credit2'],
                'credit1'=>$member['credit1'],

                'mobile'=>$member['mobile'],
                'mobileverify'=>$member['mobileverify'],
                'weixin'=>$member['weixin'],
                'isblack'=>$member['isblack'],
                'content'=>$member['content'],
                'createtime'=>date('Y-m-d H:i:s', $member['createtime']),

                'self_ordercount'=>$member['self_ordercount'],
                'self_ordermoney'=>$member['self_ordermoney'],
                'last_ordertime'=>$member['last_ordertime'],

                'level'=>$member['level'],
                'levelname'=>$level_title,
                'groupid'=>$member['groupid'],
                'groupname'=>$group_title,
                'agentlevel'=>$member['agentlevel'],
                'agentlevelname'=>$comlevel_title,
                'fixagentid'=>$member['fixagentid'],
                'isagent'=>$member['isagent'],
                'status'=>$member['status'],
                'agentnotupgrade'=>$member['agentnotupgrade'],
                'commission_total'=>$member['commission_total'],
                'commission_pay'=>$member['commission_pay'],
                'agenttime'=>$member['agenttime'] > 0 ? $member['agenttime'] : '非分销商',
                'agentselectgoods'=>$goods_title,
            ...
            'perm'=>array(
                'member_edit'=>cv('member.list.edit'),
                'recharge_credit2'=>cv('finance.recharge.credit2'),
                'recharge_credit1'=>cv('finance.recharge.credit1'),
                'commission_changeagent'=>cv('commission.agent.changeagent'),
                'commission_check'=>cv('commission.agent.check'),
                'order'=>cv('order'),
            )
        );
        ...
        if($hascommission){
            if($parentagent){
                $result['parentagent'] = array(
                    'id'=>$parentagent['id'],
                    'avatar'=>$parentagent['avatar'],
                    'nickname'=>$parentagent['nickname'],
                    'realname'=>$parentagent['realname'],
                    'mobile'=>$parentagent['mobile'],
                    'weixin'=>$parentagent['weixin'],
                );
            }
