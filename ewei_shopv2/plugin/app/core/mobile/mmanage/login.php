...
class Login_EweiShopV2Page extends AppMobilePage {
    ...
    public function submit() {
        ...
        if($type == 'wechat'){
            ...
            // ALTER TABLE `ims_ewei_shop_perm_user` ADD COLUMN `openid_wa`  varchar(50) NULL AFTER `openid`;

            $roleuser = pdo_fetch("SELECT id, uid, uniacid, username, status, openid_wa FROM". tablename("ewei_shop_perm_user"). "WHERE openid_wa=:openid LIMIT 1", array(
                ":openid"=>$openid
            ));
        ...
        // 账号密码登录
            ...
            if($role == 'operator'){
                $roleuser = pdo_fetch("SELECT id, uid, uniacid FROM". tablename("ewei_shop_perm_user"). "WHERE uid=:uid LIMIT 1", array(
                    ":uid"=>$account['uid']
                ));
                if (empty($roleuser)) {
                    return app_error(AppError::$UserLoginFail, '此账号没有管理权限');
    ...
    private function we_role($uid=0){
        ...
        // 判断副创使人
        $vice_founder =  pdo_fetchcolumn('SELECT COUNT(id)FROM '. tablename('uni_account_users'). 'WHERE uid=:uid AND role=:role', array(
            ':uid'=>$uid,
            ':role'=>'vice_founder'
        ));
        ...
        // 判断管理员
        $owner =  pdo_fetchcolumn('SELECT COUNT(id)FROM '. tablename('uni_account_users'). 'WHERE uid=:uid AND role=:role', array(
            ':uid'=>$uid,
            ':role'=>'owner'
        ));
        if(!empty($owner)){
        ...
        // 判断操作员
        $operator =  pdo_fetchcolumn('SELECT COUNT(id)FROM '. tablename('uni_account_users'). 'WHERE uid=:uid AND role=:role', array(
            ':uid'=>$uid,
            ':role'=>'operator'
        ));
        if(!empty($operator)){
        ...
        $manager =  pdo_fetchcolumn('SELECT COUNT(id)FROM '. tablename('uni_account_users'). 'WHERE uid=:uid AND role=:role', array(
            ':uid'=>$uid,
            ':role'=>'manager'
        ));
        if(!empty($manager)){
