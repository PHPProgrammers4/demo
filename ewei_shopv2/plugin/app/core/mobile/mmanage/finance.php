...
class Finance_EweiShopV2Page extends AppMobileAuthPage {
    ...
    public function main() {
        return app_json(array(
            'perm'=>array(
                'finance_recharge'=>cv('finance.log.recharge'),
                'finance_withdraw'=>cv('finance.log.withdraw'),
                //'finance_credit1'=>cv('finance.credit.credit1'),
                'finance_credit2'=>cv('finance.credit.credit2')
            )
        ));
    }
    ...
    protected function log($type) {
        ...
        $sql = "select log.id,m.id as mid, m.realname,m.avatar,log.logno,log.type,log.status,log.rechargetype,m.nickname,m.mobile,log.money,log.createtime,log.realmoney,log.deductionmoney,log.charge,log.remark,log.alipay,log.bankname,log.bankcard,log.realname as applyrealname,log.applytype,log.apppay from " . tablename('ewei_shop_member_log') . " log "
