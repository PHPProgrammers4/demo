...
class Shop_EweiShopV2Page extends AppMobilePage
{
    ...
    function get_shopindex(){
        ...
        if(!empty($_W['shopset']['shop']['indexrecommands'])){
            ...
            if(!empty($goodids)){
                ...
                foreach($indexrecommands as $key => $value){
                    $indexrecommands[$key]['marketprice'] = (float)$indexrecommands[$key]['marketprice'];
                    $indexrecommands[$key]['minprice'] = (float)$indexrecommands[$key]['minprice'];
                    $indexrecommands[$key]['presellprice'] = (float)$indexrecommands[$key]['presellprice'];
                    $indexrecommands[$key]['productprice'] = (float)$indexrecommands[$key]['productprice'];
        ...
        foreach ($newsorts as $i=>&$sortitem){
            if($sortitem['data']){
                ...
                if ($sortitem['type'] != 'search') unset($newsorts[$i]);
            }
        }

        $result = array(
            'uniacid' => $uniacid,
            'sorts' => array_values($newsorts),
    ...
    function get_recommand(){
        ...
        if (!empty($recommand['list'])){
            foreach ($recommand['list'] as &$item) {
                $item['marketprice'] = (float)$item['marketprice'];
                $item['minprice'] = (float)$item['minprice'];
                $item['presellprice'] = (float)$item['presellprice'];
    ...
    function get_set()
    {
        ...
        $merch_plugin = p('merch');$merch_data = m('common')->getPluginset('merch');
        $openmerch = $merch_plugin && $merch_data['is_openmerch'];

        $sets = array(
            'shop' => array(
                'name' => $global_set['shop']['name'],
                'logo' => tomedia($global_set['shop']['logo']),
                'description' => $global_set['shop']['description'],
                'img' => tomedia($global_set['shop']['img'])
            ),
            'share' => array(
                'title' => empty($global_set['share']['title']) ? $global_set['shop']['name'] : $global_set['share']['title'],
                'img' => empty($global_set['share']['icon']) ? tomedia($global_set['shop']['logo']) : tomedia($global_set['share']['icon']),
                'desc' => empty($global_set['share']['desc']) ? $global_set['shop']['description'] : $global_set['share']['desc'],
                'link' => empty($global_set['share']['url']) ? mobileUrl('', array('appfrom'=>1), true) : $global_set['share']['url']
            ),
            'trade'=>array(
                'closerecharge'=>intval($global_set['trade']['closerecharge']),
                'minimumcharge'=>floatval($global_set['trade']['minimumcharge']),
                'withdraw'=>intval($global_set['trade']['withdraw']),
                'withdrawmoney'=>floatval($global_set['trade']['withdrawmoney']),
                'closecomment'=>intval($global_set['trade']['withdraw']),
                'closecommentshow'=>intval($global_set['trade']['closecommentshow'])
            ),
            'payset'=>array(
                'weixin'=> intval($global_set['pay']['weixin']),
                'alipay'=> intval($global_set['pay']['alipay']),
                'credit'=> intval($global_set['pay']['credit']),
                'cash'=> intval($global_set['pay']['cash']),
            ),
            'contact'=>array(
                'phone'=>isset($global_set['contact']['phone'])?$global_set['contact']['phone']:'',
                'province'=>isset($global_set['contact']['phone'])?$global_set['contact']['province']:'',
                'city'=>isset($global_set['contact']['phone'])?$global_set['contact']['city']:'',
                'address'=>isset($global_set['contact']['phone'])?$global_set['contact']['address']:'',
            ),
            'menu'=>$this->model->diyMenu('shop'),

            'cancelorderreasons' => array(
                "不取消了",
                "我不想买了",
                "信息填写错误，重新拍",
                "同城见面交易",
            ...
            'openmerch'=>$openmerch,
            'texts' => array(
                'credittext' => $global_set['trade']['credittext'],
                'moneytext' => $global_set['trade']['moneytext'],
            )
