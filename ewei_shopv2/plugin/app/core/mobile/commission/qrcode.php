...
class Qrcode_EweiShopV2Page extends Base_EweiShopV2Page {
	...
	function main() {
		...
		if (!empty($goodsid)) {
			...
			if (!empty($goods)) {
				...
				$_W['shopshare'] = array(
					...
					'desc' => !empty($goods['description']) ? $goods['description'] : (empty($set['closemyshop']) ? $myshop['name'] : $_W['shopset']['shop']['name']),
		...
		if (!$share_goods) {
				...
				if (empty($set['closemyshop'])) {
					$_W['shopshare'] = array(
						...
						'title' => $myshop['name'],
						'desc' => $myshop['desc'],
						'link' => mobileUrl('commission/share', array('mid' => $member['id']), true)
					);
				} else {
					$_W['shopshare'] = array(
						'imgUrl' => !empty($share_set['icon']) ? $share_set['icon'] : $_W['shopset']['shop']['logo'],
						'title' => !empty($share_set['title']) ? $share_set['title'] : $_W['shopset']['shop']['name'],
						'desc' => !empty($share_set['desc']) ? $share_set['desc'] : $_W['shopset']['shop']['description'],
						'link' => mobileUrl('commission/share', array('mid' => $member['id']), true)
					);
				}
        ...
        $set_res = array(
            'texts' => $set['texts'],
            'qrcode' => $set['qrcode'],
            'qrcode_title' => $set['qrcode_title'],
            'become_child' => $set['become_child'],
            'qrcode_content' => $set['qrcode_content'],
            'qrcode_remark' => $set['qrcode_remark']
        );
		return app_json(array(
		    'qrcode_title' => (empty($set['qrcode']) || (!empty($set['qrcode'])&&empty($set['qrcode_title']))) ? '如何赚钱' : $set['qrcode_title'],
		    'qrcode_remark' => empty($set['qrcode']) || (!empty($set['qrcode'])&&empty($set['qrcode_remark'])),
		    'set'=>$set_res
        ));
	}
}
