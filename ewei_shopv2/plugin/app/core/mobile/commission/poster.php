...
class Poster_EweiShopV2Page extends Base_EweiShopV2Page {
    ...
    public function getimage() {
        ...
        $poster = pdo_fetch('SELECT * FROM '. tablename('ewei_shop_wxapp_poster'). ' WHERE id=:id AND uniacid=:uniacid LIMIT 1', array(
            ':id'=>$id,
            ':uniacid'=>$_W['uniacid']
        ));
        if(empty($poster)){
        ...
        if(is_file($filepath)){
            return app_json(array(
                'thumb'=>$this->getImgUrl($filename_thumb),
                'poster'=>$this->getImgUrl($filename),
            ));
        }
        ...
        /*
        $white = imagecolorallocate($target_thumb, 255, 255, 255);
        imagefill($target_thumb, 0, 0, $white);*/
    ...
    private function getQR($poster = array(), $member = array()) {
        ...
        // 获取二维码
        $image = p('app')->getCodeUnlimit(array(
            'scene'=>'mid='. $member['id'],
