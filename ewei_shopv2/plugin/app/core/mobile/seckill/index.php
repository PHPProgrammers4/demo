...
class Index_EweiShopV2Page extends AppMobilePage
{
    public function get_list()
    {
        ...
        $_W['shopshare'] = array(
            'title' => $share_title,
            'link' => mobileUrl('seckill', array('roomid'=>$roomid,'mid'=>$mid), true),
            'imgUrl' => tomedia($task['share_icon']),
            'desc' => $share_desc
        );
    ...
    //            'diymenu'=>$page['data']['page']['diymenu'],
                ...
                'page_title'=>$page['data']['page']['title'],
                'titlebarcolor'=>$page['data']['page']['titlebarcolor'],
            );
    ...
    public function get_goods()
    {
        ...
        foreach ($goods as &$g) {
            ...
            if ($g['hasoption']) {
                $total = 0;
                $count = 0;
                $options = pdo_fetchall("select tg.id,tg.goodsid,tg.optionid,tg.price,g.title,g.marketprice,g.productprice,tg.commission1,tg.commission2,tg.commission3,tg.total from " . tablename('ewei_shop_seckill_task_goods') . "  tg  left join " . tablename('ewei_shop_goods') . " g on tg.goodsid = g.id  where tg.timeid=:timeid and tg.taskid=:taskid and tg.timeid=:timeid  and tg.goodsid=:goodsid and  tg.uniacid =:uniacid ",
                    array(':timeid' => $time['id'], ':taskid' => $taskid, ':goodsid' => $g['goodsid'], ':uniacid' => $_W['uniacid']));
                ...
                foreach ($options as $option) {
                    ...
                    if ($option['productprice'] > $productprice) {
                ...
                $g['percent'] = $seckillinfo['percent']>100?100:$seckillinfo['percent'];
            } else {
                $g['count'] = $seckillinfo['count'];
                $g['percent'] =  $seckillinfo['percent']>100?100:$seckillinfo['percent'];
            }
    ...
    function getSeckillCount($taskid, $timeid, $goodsid, $optionid = 0, $openid = '')
    {
        ...
        foreach ($keys as $key) {
            ...
            foreach ($queue as $data) {
                ...
                if ($optionid > 0 && $key_optionid === $optionid){
                    $count++;
                }elseif($optionid == 0){
                    $count++;
                }
