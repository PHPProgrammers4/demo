...
class Log_EweiShopV2Page extends Base_EweiShopV2Page {
    ...
    function get_list() {
        ...
        $dividendcount = pdo_fetchcolumn('select sum(dividend) from '.tablename('ewei_shop_dividend_apply') ." where mid=:mid and uniacid=:uniacid and status>-1 limit 1",
...
//            'set'=>$this->set,
            ...
            'list'=>$list,
            'pagesize'=>$psize,
            'dividendcount'=>number_format($dividendcount, 2)
        );
    ...
    function orders() {
        ...
        $list = pdo_fetchall("select id,headsid, ordersn,price,goodsprice, dispatchprice,createtime, paytype,dividend,dividend_status,dividend_content from " . tablename('ewei_shop_order')
            . " where  id in ( " . $apply['orderids'] . " ) LIMIT " . ($pindex - 1) * $psize . ',' . $psize);
        ...
        foreach ($list as &$row) {
            ...
            $orderpay = 0;
            $goods = pdo_fetchall("SELECT og.id,g.thumb,og.price,og.realprice, og.total,g.title,o.paytype,og.optionname from " . tablename('ewei_shop_order_goods') . " og"
