...
class Order_EweiShopV2Page extends Base_EweiShopV2Page
{
    ...
    public function main(){
        ...
        $ordercount = $member['ordercount0']; //分红订单数
        ...
        if(!empty($list)){
            foreach($list as &$row){
                ...
                $goods = pdo_fetchall('select og.id,og.goodsid,g.thumb,og.price,og.optionname,g.title from '.tablename('ewei_shop_order_goods').' og '
                    .' left join '.tablename('ewei_shop_goods').' g on g.id = og.goodsid'
                    .' where og.orderid = :orderid and og.uniacid = :uniacid order by og.createtime desc',array(':orderid'=>$row['id'],':uniacid'=>$_W['uniacid']));
        ...
        $result = array(
            ...
            'textdividend' => $this -> set['texts']['dividend']
        );
