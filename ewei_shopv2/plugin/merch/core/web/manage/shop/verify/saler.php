...
class Saler_EweiShopV2Page extends MerchWebPage {
    ...
    function main() {
        ...
        $sql = "SELECT s.*,m.nickname,m.avatar,m.mobile,m.realname,store.storename FROM " . tablename('ewei_shop_merch_saler') . "  s "
            . " left join " . tablename('ewei_shop_member') . " m on s.openid=m.openid and m.uniacid = s.uniacid "
            . " left join " . tablename('ewei_shop_merch_store') . " store on store.id=s.storeid "
    ...
    function query() {
        ...
        $ds = pdo_fetchall("SELECT s.*,m.nickname,m.avatar,m.openid,m.mobile,m.realname,store.storename FROM " . tablename('ewei_shop_merch_saler') . "  s "
                . " left join " . tablename('ewei_shop_member') . " m on s.openid=m.openid "
                . " left join " . tablename('ewei_shop_merch_store') . " store on store.id=s.storeid "
