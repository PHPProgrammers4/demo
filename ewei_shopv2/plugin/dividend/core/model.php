...
//            $dividend_withdraw = 0; //提现笔数
            ...
            if (in_array('total', $options)) {
                $order_all = pdo_fetchall('select id,headsid,price,deductcredit2,dispatchprice,dividend from '.tablename('ewei_shop_order')
                    .' where 1 and headsid = :headsid and status >= 1 and uniacid = :uniacid',array(':headsid'=>$member['id'],':uniacid'=>$_W['uniacid']));
                if(!empty($order_all)){
            ...
            if(in_array('pay', $options)){
                $check_apply_pay = pdo_fetchall('select id,headsid,price,deductcredit2,dispatchprice,dividend,finishtime from '.tablename('ewei_shop_order')
                    .' where 1 and headsid = :headsid and status >= 3 and dividend_status=3 and ('.$time.'-finishtime > '.$day_times.') and uniacid = :uniacid',array(':headsid'=>$member['id'],':uniacid'=>$_W['uniacid']));
            ...
            if(in_array('wait',$options)){
                $divedend_order_wait = pdo_fetchall('select id,headsid,price,deductcredit2,dispatchprice,dividend,finishtime from '.tablename('ewei_shop_order')
                    .' where 1 and headsid = :headsid and status in (1,2) and ('.$time.'-finishtime > '.$day_times.') and uniacid = :uniacid',array(':headsid'=>$member['id'],':uniacid'=>$_W['uniacid']));
