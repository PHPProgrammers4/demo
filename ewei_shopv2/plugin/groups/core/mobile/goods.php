...
//阶梯团
        ...
        $url = array(
            'pay' => mobileUrl('groups/goods/pay'),
            'lottery' => mobileUrl('groups/goods/lottery')
        );
        //分享
        $set = $_W['shopset'];
        $_W['shopshare'] = array(
            ...
            'link' => mobileUrl('groups/goods',array('id'=>$id),true),
        );
    ...
    function openGroups(){
        ...
        foreach($teams as $key => $value){
            $value['fightnum'] = pdo_fetchcolumn("select count(1) from ".tablename('ewei_shop_groups_order')." where goodid = :goodid and uniacid = :uniacid and deleted = 0 and is_team = 1 and status > 0 ",
                array(":goodid"=>$value['id'],':uniacid'=>$uniacid));
        ...
        $_W['shopshare'] = array(
            'title' => '我参加了“'.$goods['title'].'”拼团，快来吧。盼你如南方人盼暖气~',
            'imgUrl' => !empty($goods['share_icon']) ? tomedia($goods['share_icon']) : tomedia($goods['thumb']),
            'desc' => !empty($goods['share_title']) ? $goods['share_title'] : $goods['title'],
            'link' => mobileUrl('groups/goods',array('id'=>$id,'mid'=>$member['id']),true),
        );
        include $this->template();
    }
    /*
    ...
    function goodsCheck(){
        ...
        try{
			...
			where openid = :openid and status >= :status and goodid = :goodid and uniacid = :uniacid",
                array(':openid'=>$openid,':status'=>0,':goodid'=>$id,':uniacid'=>$uniacid));
            ...
            $order = pdo_fetch("select * from " . tablename('ewei_shop_groups_order') . '
                where goodid = :goodid and status >= 0  and openid = :openid and uniacid = :uniacid and success = 0  and is_team = 1 and deleted = 0 ',
                array(':goodid' => $id, ':openid' => $openid, ':uniacid' => $uniacid));
...
//                        如果某一阶梯没有团就不显示此阶梯
            ...
            show_json(1,array('ladder'=>$ladder,'specArr'=>$specArr));//array('list'=>$orders,'pagesize'=>$psize,'total'=>$total)
        }catch(Exception $e){
