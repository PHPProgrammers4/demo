...
class Index_EweiShopV2Page extends WebPage
{
	...
	public function batch_update()
	{
		...
		require_once("1688/Model.class.php");
		// $appKey = '9984559';
		// $appSecret = 'lCgLGTZNppr';
		$appKey = '8999921';
		$appSecret = 'dd01wVgDixm';
		$url = 'http://gw.open.1688.com/openapi/';
		$info = "param2/1/com.alibaba.product/alibaba.agent.product.simple.get/";
		$api = new Model();
		...
		$data = $api->getGoodslist($ids);
		
		$goodsInfo = $api->getApidata($data,$appKey,$appSecret,$info,$url);
		
		foreach($goodsInfo as $key=>$val){
			$arr['status'] = $val['status'];
			$arr['total'] = $val['total'];
			$arr['deleted'] = $val['deleted'];
			if($val['total'] < 200){
				$arr['status'] = 0;
			}

			if(!empty($val['skuInfos'])){
				//suk规格数组多于1组，开启多规格，并存储多规格。否则关闭多规格
				if(count($val['skuInfos']) > 1){
					$arr['hasoption'] = 1;
				
					$api->updateSku($val['skuInfos'],$val['id'],$data[$key]['productID']);
				}else{
					$arr['hasoption'] = 0;
				}
			}
			...
			$result[] = pdo_update('ewei_shop_goods', $arr, array('id' => $val['id']));
		}
