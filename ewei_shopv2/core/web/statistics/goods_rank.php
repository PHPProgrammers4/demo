...
class Goods_rank_EweiShopV2Page extends WebPage {
    ...
    function main() {
        ...
        $sum_goods = "SELECT og.goodsid, SUM( og.total ) ordercount,SUM( og.price ) money FROM ".tablename('ewei_shop_order_goods');
        $sum_goods .= " og LEFT JOIN ".tablename('ewei_shop_order'). " o ON og.orderid=o.id ";
        $sum_goods .= " WHERE og.uniacid = {$_W['uniacid']} AND og.goodsid=:goodsid AND (o.status >= 1 OR o.refundid > 0) GROUP BY goodsid";
  
        foreach ($list as $key => $row) {
            if (intval($row['status']) > 0 && intval($row['checked']) == 0 && intval($row['total']) > 0 && intval($row['deleted']) == 0) {
                $list[$key]['goodsstatus'] = '出售中';
            } else if ($row['status'] > 0 && $row['total'] <= 0 && $row['deleted'] == 0 && $row['type'] != 30) {
                $list[$key]['goodsstatus'] = '已售罄';
            } else if (($row['status'] == 0 || $row['checked'] == 1) && $row['deleted'] == 0) {
                $list[$key]['goodsstatus'] = '仓库中';
            } else if ($row['deleted'] == 1) {
                $list[$key]['goodsstatus'] = '回收站';
            } else if ($row['deleted'] == 0 && $row['merchid'] > 0 && $row['checked'] == 1) {
                $list[$key]['goodsstatus'] = '待审核';
            }
        }
