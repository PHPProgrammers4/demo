...
class Saler_EweiShopV2Page extends WebPage {
    ...
    function main() {
        ...
        $sql = "SELECT s.*,m.nickname,m.avatar,m.realname,store.storename FROM " . tablename('ewei_shop_saler') . "  s "
    ...
    function query() {
        ...
        $ds = pdo_fetchall("SELECT s.*,m.nickname,m.avatar,m.mobile,m.realname,store.storename,m.openid FROM " . tablename('ewei_shop_saler') . "  s "
