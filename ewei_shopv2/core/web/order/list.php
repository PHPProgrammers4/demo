...
';
			...
			foreach ($list as &$r) {
				...
				foreach ($ogoods as $k => $g) {
					...
					$res_multi_arr = pdo_fetchall('select applyprice,rtype from ' . tablename('ewei_shop_order_single_refund') . ' where id=:id and uniacid=:uniacid limit 1', array(':id' => $g['single_refundid'], ':uniacid' => $_W['uniacid']));
					if (!empty($r['refundstate']) && $r['refundstate'] == 3) {
						...
						if (is_array($res_multi_arr) && !empty($res_multi_arr[0])) {
							...
							$refundstatus = !empty($g['single_refundid']) ? $r_type[$res_multi_arr[0]['rtype']] . '申请' : '';
						}
				...
				if ($status === '' && empty($_GPC['keyword']) && empty($_GPC['time'])) {
					...
					if (!is_error($redis)) {
						if ($redis->get($redis_key_total) != false && $redis->get($redis_key_totalmoney) != false) {
							...
							if (!empty($heads)) {
								$redis->set($redis_key_total, $t['count'], array(0 => 'nx', 'ex' => '300'));
								$redis->set($redis_key_totalmoney, $t['sumprice'], array(0 => 'nx', 'ex' => '300'));
							}
