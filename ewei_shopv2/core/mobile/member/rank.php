...
class Rank_EweiShopV2Page extends MobileLoginPage {
	...
	public function order_rank()
	{
		...
		/*$result_one = pdo_fetch("SELECT SUM(price) as price,openid FROM ".tablename('ewei_shop_order')."
			WHERE uniacid = :uniacid AND openid = :openid AND status = 3 GROUP BY openid ",
			array(':uniacid'=>$_W['uniacid'],':openid'=>$user['openid']));*/

		/*$result_all = pdo_fetchall("SELECT SUM(price) as price,openid FROM ".tablename('ewei_shop_order')."
	...
	public function ajaxorderpage()
	{
		...
		/*$result = pdo_fetchall("SELECT m.id,m.uid,m.nickname,m.avatar,SUM(o.price) as price FROM ".tablename('ewei_shop_member')." as m
		JOIN ". tablename('ewei_shop_order') ." as o ON m.openid = o.openid AND m.uniacid = o.uniacid
