...
class Uploader_EweiShopV2Page extends MobilePage
{
	public function main()
	{
		...
		{
            $_FILES[$field] = implode(",", $_FILES[$field]);
            $data = $_FILES[$field];

            var_dump($data);

            $url =  m('common')->post_alibaba_api('','','param2/1/com.alibaba.trade/alibaba.trade.uploadRefundVoucher/',$data);
            $buyerView = m('common')->curl_post($data,$url);

            var_dump($buyerView);die;
