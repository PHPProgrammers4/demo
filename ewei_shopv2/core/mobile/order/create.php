...
//		$json = '{"address":"网商路699号","phone": "0517-88990077","mobile": "15251667788","fullName": "张三","postCode": "000000","areaText": "滨江区","townText": "","cityText": "杭州市","provinceText": "浙江省"';
//
//        var_dump($json);die;
		$ser = unserialize($order['address']);
//
//
...
//        $offer['quantity'] = ;
        ...
        $offerId = substr($allgoods['goodssn'],30,-5);
        $offerId = strrchr($allgoods['goodssn'], "/");
        $offerId = $allgoods;
        $offer = [];
        $outerOrderInfo = [];
        foreach($allgoods as $key => $value ){
            ...
            $offerId = substr(strrchr($value['goodssn'], "/"), 1,-5);
            $option = pdo_get('ewei_shop_goods_option',array('uniacid'=>$_W['uniacid'],'id'=>$value['optionid']));
            $offer[$key]['offerId'] = $offerId;
            $offer[$key]['specId'] = $option['specid'];
            $offer[$key]['quantity'] = $value['total'];


            $outerOrderInfo['mediaOrderId'] = $order['ordersn'];
            $outerOrderInfo['phone'] = $ser['mobile'];
            $outerOrderInfo['offers'][$key]['id'] = $offerId;
            $outerOrderInfo['offers'][$key]['specId'] = $option['specid'];
            $outerOrderInfo['offers'][$key]['price'] = $order['price'];
            $outerOrderInfo['offers'][$key]['num'] = $value['total'];
        ...
        $address = [];
        $address['fullName'] = $ser['realname'];
        $address['mobile'] = $ser['mobile'];
        $address['phone'] = $ser['mobile'];
        $address['postCode'] = explode(' ',$ser['datavalue'])[2];
        $address['cityText'] = $ser['city'];
        $address['provinceText'] = $ser['province'];
        $address['areaText'] = $ser['area'];
        $address['townText'] = '';
        $address['address'] = $ser['address'];
        $address['districtCode'] = explode(' ',$ser['datavalue'])[2];

        $data['addressParam'] = json_encode($address);
        $data['cargoParamList'] = json_encode($offer);
        $data['message'] = '123';
        $data['outerOrderInfo'] = json_encode($outerOrderInfo);
        $data['tradeType'] = 'fxassure';
        $url =  $this->post_alibaba_api($this->access_token, $this->appSecret, 'param2/1/com.alibaba.trade/alibaba.createOrder.preview4CybMedia/' . $this->appkey, $data);
...
//        var_dump($adata);die;
        ...
        $url =  $this->post_alibaba_api($this->access_token, $this->appSecret, 'param2/1/com.alibaba.trade/alibaba.trade.createOrder4CybMedia/' . $this->appkey, $data);
        ...
        $order['1688_orderId'] = $adata['result']['orderId'];
        $order['1688_totalSuccessAmount'] = $adata['result']['totalSuccessAmount']/100;
        $order['1688_postFee'] = $adata['result']['postFee']/100;
