...
class Order_EweiShopV2Model
{
	...
	public function getTotals($merch = 0)
	{
		...
		$totals['status4'] = pdo_fetchcolumn('SELECT COUNT(1) FROM ' . tablename('ewei_shop_order') . '' . (' WHERE uniacid = :uniacid ' . $condition . ' and ismr=0  and (refundstate>0 and refundid<>0 or (refundtime=0 and refundstate=3)) and deleted=0'), $paras);
